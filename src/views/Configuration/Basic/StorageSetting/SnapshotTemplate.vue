...
import {
  GetSnapshotTemplateApi,
  AddSnapshotTemplateApi,
  UpdateSnapshotTemplateApi,
  DeleteSnapshotTemplateApi,
  GetPublicHolidayApi,
  AddPublicHolidayApi,
...
interface TemplateItem {
  ...
  Resolution: string   // composite "WxH" for dropdown binding
}
...
function mapTemplateName(raw: string): string {
  const map: Record<string, string> = {
    'Not recording':              t('Setting.set_donot_img'),
    'Snapshot always':            t('Setting.set_continue_capture_img'),
    'Motion recording':           t('Setting.set_motion_snapshot'),
    'Object recording':           t('Setting.set_obj_snapshot'),
    'Motion & Object recording':  t('Setting.set_motion_and_obj_snapshot'),
    'Snapshot always 30 day':     t('Setting.set_snapshot_30'),
    'Snapshot always 60 day':     t('Setting.set_snapshot_60'),
    'Snapshot always 90 day':     t('Setting.set_snapshot_90'),
    'Snapshot always 365 day':    t('Setting.set_snapshot_365'),
  }
  return map[raw] ?? raw
}

async function loadTemplates() {
  const res = await GetSnapshotTemplateApi()
  if (res.data.msg !== 'Success') return
  templates.value = res.data.result.map((d: any) => ({
    id: d.snapshotTemplateId,
    name: mapTemplateName(d.snapshotTemplateName),
    timeRange: d.setting.templateData,
    uuid: d.uuid,
    snapshotTemplateName: d.snapshotTemplateName,
    scheduleSnapshot: d.scheduleSnapshot,
    snapshotWidth: d.snapshotWidth,
    snapshotHeight: d.snapshotHeight,
    snapshotQuality: d.snapshotQuality,
    snapshotIntervalInSec: d.snapshotIntervalInSec,
    snapshotExpireInDay: d.snapshotExpireInDay,
    Resolution: `${d.snapshotWidth}x${d.snapshotHeight}`,
  }))
}

async function addTemplate() {
  const res = await AddSnapshotTemplateApi({
    snapshotTemplateName: t('Liveview.live_customization') + '1',
    scheduleSnapshot: 0,
    snapshotWidth: 0,
    snapshotHeight: 0,
    snapshotQuality: 'HFS_SNAPSHOT_QUALITY_MEDIUM',
    snapshotIntervalInSec: 3,
    snapshotExpireInDay: 0,
...
async function deleteTemplate() {
  ...
  const res = await DeleteSnapshotTemplateApi({ ids: [clickItem.value.id] })
  if (res.data.msg === 'Success') {
...
async function confirmRename(item: TemplateItem) {
  const res = await UpdateSnapshotTemplateApi({
    uuid: item.uuid,
    snapshotTemplateName: item.name,
    scheduleSnapshot: item.scheduleSnapshot,
    snapshotWidth: item.snapshotWidth,
    snapshotHeight: item.snapshotHeight,
    snapshotQuality: item.snapshotQuality,
    snapshotIntervalInSec: item.snapshotIntervalInSec,
    snapshotExpireInDay: item.snapshotExpireInDay,
  ...
  if (item.timeRange && item.timeRange.length > 0) {
    for (const dayData of item.timeRange) {
      for (const slot of dayData.recordingRange) {
        ...
        if (dayData.dayIndex === 8) rowIdx = 7
    ...
    if (item.id < 5) {
      ...
      for (let rowIdx = item.timeRange.length; rowIdx < 8; rowIdx++) {
        for (let c = 0; c < 48; c++) applyCell(c, rowIdx, notRecordSlot)
      }
    }
  }
}
...
function onCellMouseUp(colIdx: number, rowIdx: number) {
  ...
  for (let r = rStart; r <= rEnd; r++) {
    for (let c = cStart; c <= cEnd; c++) paintCell(c, r)
  }
...
function closeCopyPopover(rowIdx: number) { copyPopoverVisible.value[rowIdx] = false }
...
async function saveTemplate() {
  ...
  const [wStr, hStr] = (clickItem.value.Resolution || '0x0').split('x')
  const res = await UpdateSnapshotTemplateApi({
...
async function resetTemplate() {
  ...
  const res = await UpdateSnapshotTemplateApi({
    uuid: clickItem.value.uuid,
    snapshotTemplateName: clickItem.value.name,
    scheduleSnapshot: clickItem.value.scheduleSnapshot,
    snapshotWidth: clickItem.value.snapshotWidth,
    snapshotHeight: clickItem.value.snapshotHeight,
    snapshotQuality: clickItem.value.snapshotQuality,
    snapshotIntervalInSec: clickItem.value.snapshotIntervalInSec,
    snapshotExpireInDay: clickItem.value.snapshotExpireInDay,
    setting: { templateData: [] },
  })
  if (res.data.msg === 'Success') {
    clearGrid()
...
function ChangeHolidy() {
  ...
  switch (holiday.value) {
    ...
    case '2': { const d = getSpringFestivalDate(Year.value, '春节'); Month.value = d ? Number(d.split('-')[1]) : 2; break }
    case '3': { const d = getSpringFestivalDate(Year.value, '清明节'); Month.value = d ? Number(d.split('-')[1]) : 4; break }
...
function clickToday() { Year.value = new Date().getFullYear(); Month.value = new Date().getMonth() + 1 }

// ─── Table header & draw options ─────────────────────────
const tableHeader = Array.from({ length: 24 }, (_, i) => String(i).padStart(2, '0'))

const drawTypes = computed(() => [
  { value: 'on',           color: '#0399FE', label: t('Setting.set_continue_capture_img') },
  { value: 'false',        color: '#292929', label: t('Setting.set_donot_img') },
...
const qualityOptions = computed(() => [
  { label: t('Setting.set_high'),   value: 'HFS_SNAPSHOT_QUALITY_HIGH' },
  { label: t('Setting.set_middle'), value: 'HFS_SNAPSHOT_QUALITY_MEDIUM' },
  { label: t('Setting.set_low'),    value: 'HFS_SNAPSHOT_QUALITY_LOW' },
])

const resolutionOptions = computed(() => [
  { label: t('Setting.set_auto'), value: '0x0' },
  { label: '640 x 320',          value: '640x320' },
  { label: '704 x 576',          value: '704x576' },
      ...
      <template v-if="isTemplateSelected">
            ...
            >
              <tr v-for="(row, rIdx) in rowUnit" :key="rIdx">
                <td class="row-label">
                  ...
                  <template v-else>{{ t('Common.comm_public_holiday') }}</template>
                </td>
                <td
        ...
        <div class="settings-form" v-if="clickItem">
          <el-form :model="clickItem" inline label-width="150px" label-position="left">
            <el-form-item :label="t('Setting.set_snapshot_delay')" style="width:350px;">
              <div class="input-with-hint">
                <el-input v-model="clickItem.scheduleSnapshot" style="width:170px">
                  <template #suffix>{{ t('Common.comm_s') }}</template>
                </el-input>
                <p class="expire-hint">（{{ t('Setting.set_0_auto') }}）</p>
              </div>
            </el-form-item>
            <el-form-item :label="t('Setting.set_resolution')" style="width:350px;">
              <el-select v-model="clickItem.Resolution" style="width:170px;">
                <el-option v-for="opt in resolutionOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
              </el-select>
            </el-form-item>
            <el-form-item :label="t('Setting.set_img_quality')" style="width:350px;">
              <el-select v-model="clickItem.snapshotQuality" style="width:170px;">
                <el-option v-for="opt in qualityOptions" :key="opt.value" :label="opt.label" :value="opt.value" />
              </el-select>
            </el-form-item>
            <el-form-item :label="t('Setting.set_snapshot_time_interval')" style="width:350px;">
              <el-input v-model="clickItem.snapshotIntervalInSec" style="width:170px">
                <template #suffix>{{ t('Common.comm_s') }}</template>
              </el-input>
            </el-form-item>
            <el-form-item :label="t('Setting.set_snapshot_expire_day')" style="width:350px;">
              <div class="input-with-hint">
                <el-input v-model="clickItem.snapshotExpireInDay" style="width:170px">
        ...
        <div class="holiday-list">
          ...
          <div class="holiday-list-content">
            <template v-for="item in tableData" :key="item.publicHolidayId">
              <div v-for="(h, idx) in item.publicHoliday.publicHoliday" :key="h.holiday" class="holiday-entry">
                <span>{{ item.year }}-{{ h.holiday }}</span>
                <i class="iconfont icon-lajitong del-icon" @click="DeletePublicHoliday(item, h, idx)"></i>
              </div>
            </template>
          </div>
        </div>
  ...
  .tpl-left {
    ...
    .tpl-left-list {
      ...
      .tpl-list-item {
        ...
        .tpl-name-btn {
          flex: 1; padding: 0 0 0 15px; font-size: 13px; color: #fff;
...
.calendar-table {
  ...
  tr, td, th { font-size: 12px; text-align: center; min-width: 25px; line-height: 1.8em; transition: background 200ms ease; }
  thead td { background: #353535; }
  .row-label-cell { width: 100px; background: #353535; }
  tbody tr { height: 30px; }
  td.row-label { background: #353535; width: 100px; white-space: nowrap; font-size: 12px; text-align: center; }
...
.copy-col {
  ...
  .copy-row {
    ...
    align-items: center;
    justify-content: center;
    :deep(.el-popover__reference-wrapper) { display: flex; align-items: center; justify-content: center; }
...
.copy-popover-body {
  .copy-day-grid {
    ...
    .copy-day-item {
      ...
      align-items: center;
      width: 90px;
      span { height: 30px; line-height: 30px; text-align: center; font-size: 12px; }

      .day-check-btn.el-button {
        width: 64px; height: 30px; line-height: 5px;
        border: 0 !important; border-radius: 0 !important; padding: 0 !important;
        background-color: #353535;
        &.checked     { background-color: #0399FE !important; }
        &.is-disabled { opacity: 0.4; cursor: not-allowed; }
      }
    }
  }
  .copy-footer { display: flex; justify-content: center; height: 45px; align-items: center; gap: 10px; }
}
</style>
...
.draw-type-bar {
  ...
  .stream-select { width: 16%; height: 100%; display: flex; align-items: center; font-size: 13px; flex-shrink: 0; }
  ...
  .type-legend {
    ...
    .legend-item {
      ...
      .legend-color {
        width: 25px; height: 25px; border-radius: 5px; margin: 0 30px;
...
.settings-form {
  ...
  .expire-hint { position: absolute; bottom: -23px; font-size: 12px; color: #FE5003; white-space: nowrap; }
  .input-with-hint { display: flex; flex-direction: column; }
}
...
.holiday-dialog {
  ...
  .holiday-list {
    ...
    .holiday-list-content {
      ...
      &::-webkit-scrollbar-thumb { border-radius: 5px; box-shadow: inset 0 0 5px rgba(218,218,218,0.2); background: rgba(218,218,218,0.2); }
      &::-webkit-scrollbar-track { box-shadow: inset 0 0 5px rgba(218,218,218,0.2); background: rgba(218,218,218,0.1); }
      .holiday-entry {
